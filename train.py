...
def main():
    ...
    model = load_model(model, in_arg.arch, in_arg.hidden_units, in_arg.lr)
...
def load_model(model, arch, hidden_units, lr):
    if arch == 'vgg19': 
        input_size = 25088
    elif arch == 'alexnet':
        input_size = 9216
    ...
    classifier= nn.Sequential(nn.Linear(1024,hidden_units), nn.ReLU(), nn.Linear(hidden_units, 102), nn.LogSoftmax(dim=1))
...
def train(model, epochs, lr, criterion, optimizer, testloader, vloader, cpu_gpu, start_time):
...
def save_checkpoint(in_arg.save_dir, model, optimizer, in_arg.epochs, in_arg.arch, image_datasets, in_arg.lr)):
